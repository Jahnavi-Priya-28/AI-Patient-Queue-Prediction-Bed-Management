...
from app.models.enums import QueueStatus, AppointmentStatus, PriorityLevel, UserRole
from app.models.user import User
from app.schemas.domain import CheckInRequest


def _staff_can_prioritize(user: User) -> bool:
    return user.role in {UserRole.RECEPTIONIST, UserRole.HOSPITAL_ADMIN, UserRole.SUPER_ADMIN, UserRole.DOCTOR}
...
def check_in_patient(db: Session, req: CheckInRequest, user: User) -> QueueEntry:
    ...
    if user.role == UserRole.PATIENT and (not user.patient_profile or appt.patient_id != user.patient_profile.id):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Patients can only check in for their own appointments")

    existing_q = db.query(QueueEntry).filter(QueueEntry.appointment_id == req.appointment_id).first()
    if existing_q:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Patient is already checked in with Token #{existing_q.token_number}")

    final_priority = appt.priority
    if req.priority:
        if req.priority != PriorityLevel.NORMAL and not _staff_can_prioritize(user):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only hospital staff can set URGENT or EMERGENCY priority levels")
        final_priority = req.priority

    dept = db.query(Department).filter(Department.id == appt.department_id, Department.organization_id == appt.organization_id).first()
    dept_code = dept.code if dept else "GEN"
    today = date.today()
    q_count = db.query(func.count(QueueEntry.id)).filter(
        QueueEntry.organization_id == appt.organization_id,
    ...
    q_entry = QueueEntry(
        ...
        token_number=f"{dept_code}-{q_count + 1:03d}",
        queue_date=today,
...
def call_next_patient(db: Session, doctor_user_id: int) -> QueueEntry:
    ...
    q_entry = db.query(QueueEntry).filter(
        QueueEntry.organization_id == doc.organization_id,
        QueueEntry.doctor_id == doc.id,
        QueueEntry.queue_date == date.today(),
        QueueEntry.status == QueueStatus.WAITING,
    ).order_by(QueueEntry.priority.desc(), QueueEntry.check_in_time.asc()).first()
    if not q_entry:
...
def _get_queue_entry_for_doctor(db: Session, queue_entry_id: int, user: User) -> QueueEntry:
    query = db.query(QueueEntry).filter(QueueEntry.id == queue_entry_id)
    if user.role != UserRole.SUPER_ADMIN:
        query = query.filter(QueueEntry.organization_id == user.organization_id)
    q_entry = query.first()
    if not q_entry:
        raise HTTPException(status_code=404, detail="Queue entry not found")
    if user.role == UserRole.DOCTOR and (not user.doctor_profile or q_entry.doctor_id != user.doctor_profile.id):
        raise HTTPException(status_code=403, detail="Doctors can only manage their assigned queue")
    return q_entry


def start_consultation(db: Session, queue_entry_id: int, user: User) -> QueueEntry:
    q_entry = _get_queue_entry_for_doctor(db, queue_entry_id, user)
...
def complete_consultation(db: Session, queue_entry_id: int, user: User) -> QueueEntry:
    q_entry = _get_queue_entry_for_doctor(db, queue_entry_id, user)
...
def get_live_queue(db: Session, user: User, department_id: Optional[int] = None, doctor_id: Optional[int] = None, queue_date: Optional[date] = None) -> List[QueueEntry]:
    ...
    if user.role == UserRole.DOCTOR and user.doctor_profile:
        query = query.filter(QueueEntry.doctor_id == user.doctor_profile.id)
    if user.role == UserRole.PATIENT and user.patient_profile:
        query = query.filter(QueueEntry.patient_id == user.patient_profile.id)
    if department_id:
